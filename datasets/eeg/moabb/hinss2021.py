...
class Hinss2021(PreprocessedDataset):
    ...
    def data_path(
        ...
        # ensure metadata has expected schema
        if not isinstance(metadata, dict):
            raise ValueError("Zenodo metadata response is not a dict; cannot parse files.")
        files_meta = metadata.get('files', [])
        if not isinstance(files_meta, list):
            log.warning("Zenodo metadata has no 'files' list; skipping download.")
            files_meta = []

        fnames = []
        for record in files_meta:
            # Be robust to variant filename keys in Zenodo response
            fname = record.get('key') or record.get('filename') or record.get('name')
            if not isinstance(fname, str):
                log.warning(f"Skipping record with unexpected filename schema: {record.keys()}")
                continue
            ...
            # Be robust to missing 'type' field in Zenodo file metadata.
            # Prefer filename-based check to avoid KeyError when 'type' is absent.
            if fname.endswith('.zip') and fname == f"P{subject:02d}.zip":
                ...
                # ensure extracted content exists even if the zip was present from a previous run
                unzdir = f"{fpath}.unzip"
                if (not os.path.isdir(unzdir)) or (len(os.listdir(unzdir)) == 0):
                    try:
                        with zipfile.ZipFile(fpath, 'r') as zf:
                            zf.extractall(unzdir)
                    except Exception as e:
                        log.error(f"Failed to extract '{fpath}' to '{unzdir}': {e}")
                        continue
        ...
        if len(fnames) == 0:
            raise FileNotFoundError(
                f"No EEGLAB '.set' files found for subject P{subject:02d} under {path}. "
                "Ensure the zip is downloaded and extracted, and files are intact."
            )
        return fnames
    ...
    def _get_single_subject_data(self, subject):
        ...
        for fn in fnames:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to read EEGLAB epochs from '{fn}'. "
                    "Verify the matching .fdt exists and is not corrupted. "
                    f"Original error: {e}"
                )
