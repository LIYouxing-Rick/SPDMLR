...
def save_results(logger,training_time,acc_val,args):
    if args.is_save:
        ...
        # unify results directory name
        torch_results_dir = './torch_results'
        if not os.path.exists(torch_results_dir):
            os.makedirs(torch_results_dir)
        # include final_acc and best_acc; append seed to filename for downstream parsing
        base_name = args.modelname.rsplit('-', 1)[0]
        save_path = os.path.join(torch_results_dir, f"{base_name}_seed{args.seed}")
        th.save({
            'acc_val': acc_val,
            'final_acc': float(final_val_acc),
            'best_acc': float(max(acc_val)) if len(acc_val) > 0 else None,
        }, save_path)